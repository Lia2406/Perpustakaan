...
void mode_simulasi(SistemPerpustakaan* perpustakaan) {
    ...
    batalkan_peminjaman(perpustakaan, "Umum1", "Buku1");
    tampilkan_status(perpustakaan);
    // Bersihkan memori
...
void mode_manual(SistemPerpustakaan* perpustakaan) {
    ...
    while (running) {
        ...
        printf("5. Tampilkan Status\n");
        printf("6. Lihat Daftar Buku & Antrian\n");
        printf("7. Keluar\n");
        printf("Pilih menu: ");
        int pilih;
        scanf("%d", &pilih);
        ...
        if (pilih == 1) {
            ...
            char nama[100];
            char judul_buku[100];
            printf("Nama anggota yang pemesanannya dibatalkan: ");
            fgets(nama, sizeof(nama), stdin);
            nama[strcspn(nama, "\n")] = 0;
            printf("Judul buku yang dibatalkan: ");
            fgets(judul_buku, sizeof(judul_buku), stdin);
            judul_buku[strcspn(judul_buku, "\n")] = 0;
            batalkan_peminjaman(perpustakaan, nama, judul_buku);
        } else if (pilih == 5) {
            tampilkan_status(perpustakaan);
        } else if (pilih == 6) {
